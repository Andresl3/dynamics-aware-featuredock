...
if __name__ == '__main__':
    ...
    def _load_ligand(path):
        """Robust ligand load. PDBBind ligands often trip RDKit's valence
        sanitizer (e.g. carboxylate/phosphate O with explicit valence 2).
        HeavyAtomsite labelling is purely geometric, so we fall back to a
        relaxed load (coordinates preserved, valence check skipped) when the
        strict parse returns None."""
        # 1) strict parse (normal path)
        m = Chem.MolFromMolFile(path, removeHs=False)
        if m is not None:
            return m
        # 2) relaxed: skip sanitization, then partial-sanitize WITHOUT the
        #    valence/property check that rejected it
        ...
        if m is not None:
            try:
                ...
                pass  # geometry is enough for HeavyAtomsite voxelisation
            return m
        # 3) last resort: the sibling .mol2 if present (relaxed)
        mol2 = path[:-4] + ".mol2" if path.endswith(".sdf") else None
        if mol2 and os.path.exists(mol2):
            m = Chem.MolFromMol2File(mol2, removeHs=False, sanitize=False)
            if m is not None:
                try:
                    m.UpdatePropertyCache(strict=False)
                except Exception:
    ...
    def _load_protein(path):
        """Relaxed protein load: some PDBBind protein atoms trip RDKit's
        valence sanitizer (returns None on strict parse). Voxelisation +
        HeavyAtomsite labelling are geometric, so a relaxed parse is fine."""
    ...
    try:
        prot = _load_protein(apofile)
        if prot is None:
            raise ValueError(f"protein could not be parsed even with relaxed sanitization: {apofile}")
        lig = _load_ligand(hetfile)
        if lig is None:
            raise ValueError(f"ligand could not be parsed even with relaxed sanitization: {hetfile}")
