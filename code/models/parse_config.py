...
def parser():
    ...
    parser.add_argument('--feature_per_shell', type=int, default=80, \
        help='Number of properties per FEATURE shell. 80 = baseline FeatureDock; '
             '81 = +Dyna-1 dynamics channel (augmentation 1). Must match the '
             'property tensors in --datafolder (480 vs 486 cols).')
    parser.add_argument('--num_shells', type=int, default=6, \
        help='Number of concentric FEATURE shells per grid point (default 6).')
...
def load_config(args):
    ...
    # Augmentation (1): configurable feature width. 80 = baseline, 81 = +Dyna-1.
    FPS = getattr(args, 'feature_per_shell', 80)
    NSHELL = getattr(args, 'num_shells', 6)
    CONFIG['feature_per_shell'] = FPS
    CONFIG['num_shells'] = NSHELL
    INPUT_DIM = NSHELL * FPS  # 480 (baseline) or 486 (+Dyna-1)
    print(f"Model input width: num_shells={NSHELL} x feature_per_shell={FPS} = {INPUT_DIM}")
    ################
    #  INIT SEED   #
    ################
    ...
    if args.modeltype == 'cnn':
        ...
        CONFIG['lr'] = args.lr
        model = FeatIntResNet(feature_per_shell=FPS, num_shells=NSHELL, n_blocks=CONFIG['n_blocks'], \
        ...
        CONFIG['lr'] = args.lr
        model = get_fnn_model(INPUT_DIM, CONFIG['h_dims'], CONFIG['n_class'], dropout=0.5)
        ...
        model = BertSentClassifier(n_class=CONFIG['n_class'], num_shells=NSHELL, feature_per_shell=FPS, \
